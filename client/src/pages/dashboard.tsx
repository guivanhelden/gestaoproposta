...
export default function Dashboard() {
  ...
  const { data: proposalStats, isLoading: isLoadingStats } = useQuery({
    ...
    select: (proposals) => {
      ...
      const concluidas = proposals.filter(p => 
        p.stage === 'contrato-ativo'
      ).length;
      const andamento = total - pendentes - concluidas;
      
      // Cálculo simplificado do valor de vendas do mês
      const vendasMes = proposals
        .filter(p => p.stage === 'contrato-ativo')
        .reduce((acc, p) => acc + (parseFloat(p.value?.replace('R$ ', '').replace(',', '.')) || 0), 0);
